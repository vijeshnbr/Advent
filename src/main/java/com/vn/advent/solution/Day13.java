...
public class Day13 implements Solution {
	...
	private static final Map<Coordinates, Vehicle> vehicles = new TreeMap<>(Comparator.comparing(Coordinates::getY)
		...
		while (!collisionDetected) {
			...
			for (Vehicle v : copyOfvehicles) {
				...
				if (vehicles.keySet()
					...
					// Collision detected
					System.out.println("COLLISION : " + newLocOfVehicle);
			...
			for (Vehicle v : copyOfvehicles) {
				...
				if (!allCrashesInTick.contains(locOfCurrentVehicle)) {
					...
					if (vehicles.keySet()
						...
						vehicles.put(newLocOfVehicle, afterMove);
						if (vehicles.size() == 1) {
							System.out.println(newLocOfVehicle);
						}
					}
				}
			}
	...
	static class Vehicle {
		...
		public String toString() {
			return "Vehicle [location=" + location + ", going " + direction + "]";
		}
	...
	static enum Type {
		VERTICAL('|'), HORIZONTAL('-'), FRONT_SLASH('/'), BACK_SLASH('\\'), PLUS('+'), VEHICLE_WEST('<'), VEHICLE_EAST(
				'>'), VEHICLE_NORTH('^'), VEHICLE_SOUTH('v'), UNPAVED(' ');
		private char c;
			...
			IntStream.range(0, line.length())
				.forEach(i -> {
					...
					case BACK_SLASH: {
						...
						if (west != null && (west.getType() == Type.HORIZONTAL || west.getType() == Type.PLUS)) {
							west.setEast(curve);
							curve.setWest(west);
						}
						Track north = map.get(c.top());
						if (north != null && (north.getType() == Type.VERTICAL || north.getType() == Type.PLUS)) {
					...
					case HORIZONTAL: {
						...
						Track west = map.get(c.left());
						if (west != null && (west.getType() == Type.HORIZONTAL || west.getType() == Type.PLUS
								|| west.getType() == Type.FRONT_SLASH || west.getType() == Type.BACK_SLASH)) {
							west.setEast(road);
							road.setWest(west);
						}
						Track north = map.get(c.top());
					...
					case VERTICAL: {
						...
						Track north = map.get(c.top());
						if (north != null && (north.getType() == Type.PLUS || north.getType() == Type.VERTICAL
								|| north.getType() == Type.BACK_SLASH || north.getType() == Type.FRONT_SLASH)) {
							north.setSouth(road);
							road.setNorth(north);
						}
						break;
