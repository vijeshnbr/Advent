...
public class Day12 implements Solution {
	...
	private static final Pattern FIND_PATTERNS_PATTERN = Pattern.compile("([#|\\.]{5})\\s\\=\\>\\s#");
	...
	public void partOne(Stream<String> lines) {
		...
		String prepCurrentGenPlantStateForAnalysis = prepPlantStateForAnalysis(INITIAL_STATE);
		// System.out.println();
		...
		for (long i = 1; i <= TOTAL_GEN_PART_ONE; i++) {
			...
			plantYieldingPatterns.forEach(pattern -> {
				...
				while (m.find()) {
					...
					// + (indexOfPlant - 4));
					indicesYieldingPlantsInNextGeneration.add(indexOfPlant - (4 - tmpHead));
					m.region(start + 1, currentGen.length() - 1);
				}
			});
			StringBuilder nextGen = new StringBuilder(currentGen.replaceAll("#", "."));
			indicesYieldingPlantsInNextGeneration.forEach(index -> nextGen.setCharAt(index + (4 - tmpHead), '#'));
			String prepNextGenPlantStateForAnalysis = prepPlantStateForAnalysis(nextGen.toString());
			// System.out.println(prepNextGenPlantStateForAnalysis);
			stackOfGenerations.push(prepNextGenPlantStateForAnalysis);
			IntSummaryStatistics summaryStatistics = indicesYieldingPlantsInNextGeneration.stream()
		...
		System.out.print(totalOfIndices);
	}
	...
	public void partTwo(Stream<String> lines) {
		...
		System.out.print(totalOfIndices + (TOTAL_GEN_PART_TWO - currGen) * differential);
	}
