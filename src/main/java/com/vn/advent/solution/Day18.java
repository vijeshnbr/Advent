...
public class Day18 implements Solution {
	...
	public void partOne(Stream<String> lines) {
		...
		for( int i = 1; i <= 10; i++ )
		{
			Map<Coordinates, Acre> changedLandscape = Acre.acres.values().stream().parallel().map( Acre::getNewValue ).collect( Collectors.toMap( Acre::getCoordinates, Function.identity() ) );
			Acre.acres.clear();
			Acre.acres.putAll( changedLandscape );
		}

		long countLumberYard = Acre.acres.values().stream().map( Acre::type ).filter( Type.LUMBERYARD::equals ).count();
		long countAcresWithTrees = Acre.acres.values().stream().map( Acre::type ).filter( Type.TREES::equals ).count();
	...
	static interface Acre
	{
		...
		Predicate<Acre> isGroundAndSurroundedBy3orMoreAcresWithTrees = a ->
			{
				...
				long count = Acre.neighbors( a ).stream().map( Acre::type ).filter( Type.TREES::equals ).count();
		...
		Predicate<Acre> isTreesAndSurroundedBy3orMoreLumberYards = a ->
			{
				...
				long count = Acre.neighbors( a ).stream().map( Acre::type ).filter( Type.LUMBERYARD::equals ).count();
		...
		Predicate<Acre> isLumberYardAndNotSurroundedBy1orMoreLumberYardAnd1OrMoreAcresWithTrees = a ->
			{
				...
				long countLumberYard = Acre.neighbors( a ).stream().map( Acre::type ).filter( Type.LUMBERYARD::equals ).count();
				long countAcresWithTrees = Acre.neighbors( a ).stream().map( Acre::type ).filter( Type.TREES::equals ).count();
				boolean isSurroundedBy1orMoreLumberYardAnd1OrMoreAcresWithTrees = countLumberYard >= 1 && countAcresWithTrees >= 1;
		...
		{
			return a.getCoordinates().getNeighbors()
							.stream()
							.map( acres::get )
